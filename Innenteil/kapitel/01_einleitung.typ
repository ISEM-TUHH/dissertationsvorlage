...
und die zugehörigen Dateien in `Innenteil/kapitel/` daneben legt, sieht zu
...
gespeist, `Innenteil/literatur.bib`, und kann deshalb nicht auseinanderlaufen.
...
Formelschrift. Das gilt auch für einzelne Größen im Fließtext: Die Masse $m$
und die Lichtgeschwindigkeit $c$ werden mit `$m$` und `$c$` gesetzt und
erscheinen dadurch kursiv und in derselben Schrift wie die abgesetzte Formel.
...
davon mitkommen, steht in `Innenteil/angaben.typ` — jeder Teil hat dort einen
