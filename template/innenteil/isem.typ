...
/// Wahr, solange gerade ein Verzeichnis (Abbildungen, Tabellen) gesetzt wird.
/// Die Verzeichnisse setzen die Beschriftungen erneut - ein Zitat in einer
/// Beschriftung wuerde dort ein zweites Mal ausgeloest und erzeugte eine
/// Fusznote im Verzeichnis.
#let im-verzeichnis = state("isem-im-verzeichnis", false)

/// Quellenangabe fuer eine Abbildung oder Tabelle, innerhalb der
/// Beschriftung:
///
///     caption: [Systemmodell nach #autor[Weilkiens].#bildquelle(<weilkiens2014>)]
///
/// Auf der Seite der Abbildung entsteht daraus die gewohnte Fusznote des
/// Zitierstils - die Quelle steht also dort, wo die Abbildung steht. Im
/// Abbildungsverzeichnis hinten wird sie unterdrueckt: dort gehoeren keine
/// Quellen und keine Fusznoten hin.
#let bildquelle(schluessel) = context if not im-verzeichnis.get() {
  cite(schluessel)
}
...
#let configure(meta, body) = {
  // PDF-Metadaten. Titel, Name und Schlagworte kommen aus inhalt/angaben.typ -
  // sie stehen damit auszerhalb der Vorlage und ueberleben jede Aktualisierung
  // des template-Ordners.
  set document(
    title: meta.title,
    author: meta.name,
...
#let list-page(title, art, kind-name) = [
  ...
  // Solange das Verzeichnis gesetzt wird, unterdrueckt `bildquelle()` die
  // Zitate in den Beschriftungen - Quellen stehen bei der Abbildung, nicht
  // hier.
...
/// Bilddateien liegen in inhalt/abbildungen/. Ein bloszer Dateiname wird
...
#let abbildung(rohpfad, hoehe: max-abbildungshoehe, alt: none) = layout(verfuegbar => {
  let pfad = bildpfad(rohpfad)
...
/// gebaut wird.
#let formelbild(rohpfad, alt: none) = raster-block(vor: 1, luft: 0pt, context {
  let pfad = bildpfad(rohpfad)
